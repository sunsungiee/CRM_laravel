    ...
    <div class="content tasks">
        <div class="header tasks">
            ...
            <div class="header_btns">
                <button class="add_task" id="add_task">Добавить новую задачу сотруднику</button>
        ...
        <br>
        <h2 class="admin_h">Список сотрудников</h2>
        <br><br>
        <div class="users">
            @foreach ($users as $user)
                <div class="user">
                    <h3>{{ $user->surname . ' ' . $user->name }}</h3>
                    <p style="color: gray">{{ $user->post }}</p>
                    <p>{{ $user->phone }}</p>
                    <p>{{ $user->email }}</p>
                </div>
            @endforeach
        ...
        <br><br>
        <h2 class="admin_h">Задачи сотрудников</h2>
        <br>
        <form method="GET" action="{{ route('admin.main') }}" class="search_form">
            <div class="search_container">
                <input type="text" tabindex="1" name="search" placeholder="Поиск..." id="searchInput"
                    value="{{ request('search') }}">
                <button type="submit" style="display: none;">c</button>
        ...
        <table class="tasks_table" id="contacts_table">
            <thead>
                <tr>
                    <th data-column="user_surname">
                        <a class="sort_btn"
                            href="{{ route('admin.main', ['sort' => 'user_surname', 'direction' => $direction === 'asc' ? 'desc' : 'asc']) }}">Исполнитель
                            @if ($sort === 'user_surname')
                                {{ $direction === 'asc' ? '↑' : '↓' }}
                            @endif
                        </a>
                    </th>
                    <th data-column="subject">
                        <a class="sort_btn"
                            href="{{ route('admin.main', ['sort' => 'subject', 'direction' => $direction === 'asc' ? 'desc' : 'asc']) }}">Тема
                            @if ($sort === 'subject')
                                {{ $direction === 'asc' ? '↑' : '↓' }}
                            @endif
                        </a>
                    ...
                    <th data-column="description">
                        <a class="sort_btn"
                            href="{{ route('admin.main', ['sort' => 'description', 'direction' => $direction === 'asc' ? 'desc' : 'asc']) }}">
                            Описание
                            @if ($sort === 'description')
                                {{ $direction === 'asc' ? '↑' : '↓' }}
                            @endif
                    ...
                    <th data-column="contact_surname">
                        <a class="sort_btn"
                            href="{{ route('admin.main', ['sort' => 'contact_surname', 'direction' => $direction === 'asc' ? 'desc' : 'asc']) }}">
                            Клиент
                    ...
                    <th data-column="date">
                        <a class="sort_btn"
                            href="{{ route('admin.main', ['sort' => 'date', 'direction' => $direction === 'asc' ? 'desc' : 'asc']) }}">
                            Дата
                            @if ($sort === 'date')
                                {{ $direction === 'asc' ? '↑' : '↓' }}
                            @endif
                    ...
                    <th data-column="time">
                        <a class="sort_btn"
                            href="{{ route('admin.main', ['sort' => 'time', 'direction' => $direction === 'asc' ? 'desc' : 'asc']) }}">
                            Время
                            @if ($sort === 'time')
                                {{ $direction === 'asc' ? '↑' : '↓' }}
                            @endif
                    ...
                    <th data-column="priority">
                        <a href="{{ route('admin.main', ['sort' => 'priority', 'direction' => $direction === 'asc' ? 'desc' : 'asc']) }}"
                            class="sort_btn">
                            Приоритет
                            @if ($sort === 'priority')
                                {{ $direction === 'asc' ? '↑' : '↓' }}
                    ...
                    <th data-column="status">
                        <a href="{{ route('admin.main', ['sort' => 'status', 'direction' => $direction === 'asc' ? 'desc' : 'asc']) }}"
                            class="sort_btn">
                            Статус
                            @if ($sort === 'status')
                                {{ $direction === 'asc' ? '↑' : '↓' }}
            ...
            <tbody>
                @foreach ($tasks->filter(fn($task) => !$task->trashed()) as $task)
                    <tr>
                        <td> {{ $task->user_surname }} {{ $task->user_name }} </td>
                        <td> {{ $task->subject }} </td>
    ...
    <div class="modal" id="modal">
        <div class="modal_content">
            <form action="{{ route('admin.store') }}" method="post" class="add_form">
                ...
                <fieldset>
                    <legend>Исполнитель задачи</legend>
                    <select name="user_id" id="user" class="add_select add">
                        @foreach ($users as $user)
                ...
                <fieldset>
                    ...
                    <select name="contact_id" id="contact" class="add_select add">
                        <option value="0" selected>Без клиента</option>
